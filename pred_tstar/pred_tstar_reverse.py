...
def define_arguments():
    ...
    parser.add_argument('--dists', nargs='+', type=float, help=helptext)
...
def calc_tstar_tvel(model, fnam_tvel, distance, phase, depth):
    tstar, time, p, maxdepth = calc_sens_full(model, fnam_tvel=fnam_tvel,
    ...
    if time is None:
        ...
        tstar = -1.
        p = -1.
    return tstar, time, p, maxdepth


def main(fnams_nd, dists,
         phase_list_dist,
    ...
    with open(fnam_out, 'w') as f_tstar, \
        ...
        for fnam_nd in fnams:
            ...
            for dist in dists:
                # get_dist(model, tSmP=tSmP, phase_list=phase_list_dist,
                #          plot=plot, depth=depth)
                tstar_P, time_P, ray_param_P, maxdepth_P = calc_tstar_tvel(model=model,
                                                                           fnam_tvel=fnam_nd,
                                                                           distance=dist,
                                                                           phase=phase_list_dist[0],
                                                                           depth=depth)
                tstar_S, time_S, ray_param_S, maxdepth_S = calc_tstar_tvel(model=model,
                                                                           fnam_tvel=fnam_nd,
                                                                           distance=dist,
                                                                           phase=phase_list_dist[1],
                                                                           depth=depth)

                if time_P is None or time_S is None:
                    tstar_P = -1.
                    tstar_S = -1.
                    ray_param_P = -1.
                    ray_param_S = -1.
                ...
                if maxdepth_P is None:
                    maxdepth_P = -1.
                if maxdepth_S is None:
                    maxdepth_S = -1.

                print(dist, time_S - time_P, tstar_P, tstar_S,
                      maxdepth_P, maxdepth_S)
                f_tstar.write('%5.2f %6.2f %5.3f %5.3f %8.1f %8.1f ' %
                              (dist, time_S - time_P, tstar_P, tstar_S,
                               maxdepth_P, maxdepth_S))
                f_p.write('%5.2f %6.2f %5.3f %5.3f ' %
                          (dist, time_S - time_P, ray_param_P, ray_param_S))
                f_pred.write('%5.2f %6.2f ' %
                             (dist, time_S - time_P))
